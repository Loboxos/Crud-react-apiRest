...
function App() {
  ...
  const peticionPut=async()=>{
    ...
    .then(response=>{
      var dataNueva = data;
      dataNueva.forEach(tablas_framework => {
        if (tablas_framework.id === frameworkSeleccionado.id) {
          tablas_framework.nombre = frameworkSeleccionado.nombre;
          tablas_framework.lanzamiento = frameworkSeleccionado.lanzamiento;
          tablas_framework.desarrollador = frameworkSeleccionado.desarrollador;
        }
      });
      setData(dataNueva);
  ...
  const peticionDelete=async()=>{
    ...
    await axios.post(baseUrl, f, {params: {id: frameworkSeleccionado.id}})
    .then(response=>{
      setData(data.filter(tablas_framework=>tablas_framework.id!==frameworkSeleccionado.id));
  ...
  const seleccionarFramework=(tablas_framework, caso)=>{
    setFrameworkSeleccionado(tablas_framework);
...
<br />
    ...
    <td>{val.id}</td>
    <td>{val.nombre}</td>
    <td>{val.lanzamiento}</td>
    <td>{val.desarrollador}</td>
    <td>
      <button className="btn btn-primary" onClick={() => seleccionarFramework(val, "Editar")}>Editar</button>{" "}
      <button className="btn btn-danger" onClick={() => seleccionarFramework(val, "Eliminar")}>Eliminar</button>
    </td>
  </tr>
))}
